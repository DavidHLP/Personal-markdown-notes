...
* 先操作缓存还是先操作数据库？
  ...
  * 先操作数据库，再删除缓存
    ...
    应该具体操作缓存还是操作数据库，我们应当是先操作数据库，再删除缓存，原因在于，如果你选择第一种方案，在两个线程并发来访问时，假设线程1先来（先删除了缓存，但还没有更新数据库），他先把缓存删了，此时线程2过来，他查询缓存数据（已经被线程一删除）并不存在，此时他写入缓存，当他写入缓存后（由于线程一还没有更新数据，所以写入缓存的数据是从数据库中得到的旧数据），线程1再执行更新动作时，实际上写入的就是旧的数据，新的数据被旧数据覆盖了。
